...
export async function syncAgents(projectRoot: string) {
  ...
  if (config.update_agents_md) {
    updateAgentsMd(projectRoot, false);
  }
}

export function updateAgentsMd(projectRoot: string, remove: boolean = false) {
  const agentsMdPath = path.join(projectRoot, 'AGENTS.md');
  const START_MARKER = '<!-- @generated by create-code-buddy (Start) -->';
  const END_MARKER = '<!-- @generated by create-code-buddy (End) -->';

  // Only modify AGENTS.md if it already exists — we never create it
  if (!fs.existsSync(agentsMdPath)) return;

  let content = fs.readFileSync(agentsMdPath, 'utf8');

  // Strip any existing Code Buddy block (idempotent)
  let startIndex = content.indexOf(START_MARKER);
  while (startIndex !== -1) {
    const endIndex = content.indexOf(END_MARKER, startIndex);
    if (endIndex !== -1) {
