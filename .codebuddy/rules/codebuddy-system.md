...
- **Create a rule**: Run `npx create-code-buddy add --name "folder/rule" --globs "*.ts" --description "..."`
...
- **Sync**: ALWAYS run `npx create-code-buddy sync` after manual edits to compile changes globally.
...
Example — Antigravity running non-interactively:
  npx create-code-buddy init --yes --agents gemini

Multiple agents (comma-separated, no spaces):
  npx create-code-buddy init --yes --agents gemini,cursor
